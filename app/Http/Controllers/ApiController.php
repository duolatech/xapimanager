...
class ApiController extends Controller
{
    ...
    {
        //字母分类
        $result['letter'] = array('A','B','C','D','E','F','G','H','I','J','K',
        ...
        return view('Api.list', ['list'=>$result, 'get'=>$_GET]);
    }
    /**
     * 获取 ajax Api列表
     * @return api列表
    ...
    public function ApiDetail(){
        ...
        $gtype = array_merge($type, array('HEADER'));
        ...
        if($version_type!='add' && !empty($detailid)){
            ...
            if(!empty($param['type'])){
                ...
                $gtype = array_merge($type, array('HEADER'));
                foreach ($gtype as $value){
                    $data['param'][$value]['item'] = array('type'=>$value, 'data'=>$res[$value]);
                }
                //状态码
    ...
    public function apiParamStore($data, $detailid){
        ...
        //请求及响应参数字段
        $field = array('get', 'post', 'put', 'delete');
        $request = array();
        $response = array();
        ...
        if(!empty($data['request_type'])){
            ...
            $arr = array('header', 'request', 'response', 'statuscode');
    ...
    public function publish($did, $envid){
        ...
        if(!empty($env[$key+1])){
            ...
            return response()->json(['status'=>4011, 'message'=>'环境不存在，请确认后重试']);
        }
        //获取详情信息
        ...
        if(!empty($lid) && !empty($did) && !empty($paramid)){
            return response()->json(['status'=>200, 'message'=>'同步成功']);
        }else{
    ...
    public function syncApiDetail($data, $lid){
        ...
        $field = array('listid', 'URI', 'version', 'gateway', 'local', 'description',
            'author', 'editor', 'network', 'goback' , 'auth', 'status', 'mtime', 'ctime'
        );
